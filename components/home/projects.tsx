...
const ProjectsSection = ({ isDesktop }: IDesktop) => {
  ...
  const initMobileProjectsAnimation = (
    targetSectionRef: MutableRefObject<HTMLDivElement>
  ): ScrollTrigger => {
    const scrollTrigger = ScrollTrigger.create({
      trigger: targetSectionRef.current,
  ...
  useEffect(() => {
    ...
    let mobileScrollTrigger: ScrollTrigger | undefined;
    ...
    if (isDesktop && matches) {
      ...
      // Initialize mobile pinned effect
      mobileScrollTrigger = initMobileProjectsAnimation(targetSectionRef);
    }
    ...
    return () => {
      ...
      mobileScrollTrigger && mobileScrollTrigger.kill();
    };
  ...
  return (
    <section
      ...
      className={`${isDesktop ? "min-h-screen" : "pb-20"} ${
