...
const svgColor = "#FFDADA";
const animColor = "#F65608";
...
const TimelineSection = ({ isDesktop }: IDesktop) => {
  ...
  const addText = (
    ...
    const logoString = image
      ? `<img src='${image}' class='h-8 mb-2 object-cover' loading='lazy' width='auto' height='auto' alt='${image}' />`
      : "";
  ...
  const renderSlides = (): React.ReactNode => (
    <div
      className="max-w-full h-80 shadow-xl bg-gray-800 rounded-2xl overflow-hidden"
      ref={screenContainer}
    >
      <Image
      ...
      <div className="relative h-full w-full -mt-2">
        <div className="absolute top-0 left-0 h-full w-full">
          {svgCheckpointItems.map((item, index) => (
            <Image
              className={`w-full absolute h-full top-0 object-contain slide-${
                index + 1
              }`}
  ...
  const renderSectionTitle = (): React.ReactNode => (
    <div className="flex flex-col">
      <p className="seq  text-red-900">Experience</p>
      <h1 className="text-5xl font-extrabold seq mt-2 text-red-900">Timeline</h1>
  ...
  return (
    <section
      className="w-full relative select-none min-h-screen section-container py-8 flex flex-col justify-center bg-[#C1DDCA] text-red-900"
      id={MENULINKS[3].ref}
    >
      {renderSectionTitle()}
